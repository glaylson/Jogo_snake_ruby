...
class Snake
  ...
  def draw    
    @positions.each do |position|
      Square.new(x: position[0] * GRID_SIZE, y: position[1] * GRID_SIZE  , size: 19, color: 'white')
    end
  end 
...
class Game
  def initialize
    @score = 0
    #posições aleatórias do ponto amarelo
    @ball_x = rand(GRID_WIDTH)
    @ball_y = rand(GRID_HEIGHT)
    @finished = false
  end

  #desenhar ponto amarelo
  def draw     
    unless finished?  
...
#executar método para evitar o comando no sentido contrário
